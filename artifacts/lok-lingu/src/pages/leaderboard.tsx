...
import { useGetLeaderboard, useGetLeaderboardSummary, useGetLanguages } from "@workspace/api-client-react";
    ...
    <div className="p-6 space-y-8 pt-12 pb-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-card border-border">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-2">
            ...
            <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Players</div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-2">
            ...
            <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Runs</div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border md:col-span-2">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center space-y-2">
            <Crown className="w-6 h-6 text-primary" />
            <div className={`text-2xl font-mono font-bold text-primary ${theme === 'theme-neon' ? 'neon-text-glow' : ''}`}>
              {summary?.allTimeHighCount || 0}
            </div>
            <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">All-Time High by {summary?.topPlayer || 'Unknown'}</div>
          </CardContent>
        </Card>
      </div>

      <div className="flex gap-4">
        <Select value={language} onValueChange={(v) => { setLanguage(v); setCategory("all"); }}>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-muted-foreground font-serif italic">No runs found for these filters.</div>
        ) : (
          leaderboard?.map((entry, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`font-mono font-black text-xl w-8 text-center ${
